...
<div class="first-block-content-wrapper second-page">
		<div class="first-block-content">
			...
			<?php
				...
				// 		$text = $row['text'];
				// 		echo "<p>" . $text . "</p>";
				// 		echo '<p class="p_price">' . $price . (!count($rows) ? '<a href="" class="special-offets">Снизить цену</a>' : '') . '</p>';
				// 	}
				// }
			?>
			<p>Стоимость выхода 20 секундного ролика на канале</p>
	...
	<div class="ocenka-raschet">
		...
		?>
		<h2>Оценочные расчеты</h2>
		<div class="raschet-items">
			<?php if($isLive){ ?>
				<div class="raschet-item">
					<img src="<?=get_template_directory_uri()?>/img/moscow.png" alt="">
					<button class="pricing-live-moscow">
						Эфирные каналы <span>Москва</span>
					</button>
				</div>
				<div class="raschet-item">
					<img src="<?=get_template_directory_uri()?>/img/russia.png" alt="">
					<button class="pricing-live-rf">
						Эфирные каналы <span>Россия</span>
					</button>
				</div>
			<?php }else{?>
				<div class="raschet-item">
					<img src="<?=get_template_directory_uri()?>/img/russia.png" alt="">
					<button class="pricing-thematic-rf">
						Тематические каналы <span>Россия</span>
					</button>
				</div>
			<?php }?>
		</div>
	...
	<?php
		$rows = get_field('sponsors-tv', 'option');
		if($rows && count($rows)){?>
			<div class="spnsr-block">
				<h2>Спонсорство на ТВ</h2>
	...
	<div class="five-block-content-wrapper second-page">
		<div class="five-block-text">
			...
			<p>Условия размещения рекламы по GRP в федеральных и московских рекламных блоках на данном телеканале<span>Размещение рекламных роликов по РФ и в г. Москва</span></p>
		</div>
	...
	<div class="four-block-content-wrapper second-page">
		<h2>Подберем выгодный сплит каналов</h2>
			<?php
				get_template_part( 'template-parts/common/slider', 'channels-split' );
			?>
	...
	<div class="six-block-content-wrapper">
		...
		<div class="six-block--content">
			...
			<div class="pre-footer-form-sicription">
				<p>Дополнительную информацию о программах телеканала <?=get_the_title()?>, об условиях размещения прямой, спонсорской и PR-рекламы,  а также их стоимости, Вы сможете получить по телефонам</p>
